  ...
  return (
    <nav className='w-full flex md:justify-center justify-around items-center p-4'>
      ...
      <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-initial'>
        {["Team Coding Edge"].map((item,index)=> <NavbarItem key={item+index} title={item}/>)}
      ...
      {toggleMenu && (
        ...
        >
          ...
          {["Team Coding Edge"].map((item,index)=> <NavbarItem key={item+index} title={item} classProps="my-2 text-lg"/>)}
        </ul>
      )}
      </div>
    </nav>
